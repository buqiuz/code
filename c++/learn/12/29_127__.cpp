...
//     int dist = CalcEurler(startX, startY, endX, endY);
//     que.push({startX, startY, 0, dist, nullptr});

//     while (!que.empty()) {
//         Node *cur = new Node(que.top());
//         que.pop();
...
//             int xx = cur->x + dx;
//             int yy = cur->y + dy;
//             if (xx < 1 || xx > 1000 || yy < 1 || yy > 1000 || visited.count({xx, yy}))
//                 continue;
//             int curDist = CalcEurler(xx, yy, endX, endY);
//             que.push({xx, yy, cur->g + 5, curDist, cur});
//         }
//     }
//     return -1;
// }
...
// int Astar(int startX, int startY, int endX, int endY) {
//     // static int moves[1001][1001] = {0}; //
//     如果在这定义的话，会导致stack溢出,加上static就会定义在全局变量区，不会溢出
//     // static vector<vector<int>> moves(1001, vector<int>(1001, 0));
//     // 堆上分配，不会溢出但是会超时,可以加static，然后用fill函数初始化，略慢于int数组，因为vector是动态分配的，在堆上
//     // memset(moves, 0, sizeof(moves)); // 正确清空数组
//     // for (auto &row : moves) {
...
//             // moves[xx][yy] = moves[cur.x][cur.y] + 1;
//             int curDist = CalcEurler(xx, yy, endX, endY);
//             que.push({xx, yy, cur.g + 2, curDist});
//             // }
//         }
//     }
//     return -1;
...
struct Node {
    ...
    int g, h, f;
    Ptr parent;
    // Node(int x, int y, int g, int h, Ptr parent = nullptr) : x(x), y(y), g(g), h(h), f(g + h), parent(parent) {}
    // bool operator<(const Node &k) const { return f > k.f; }
};
...
int Astar(int startX, int startY, int endX, int endY) {
    // priority_queue<Node> que;
    auto cmp = [](Ptr a, Ptr b) { return a->f > b->f; };
    priority_queue<Ptr, vector<Ptr>, decltype(cmp)> que(cmp);

    set<pair<int, int>> visited;
    int dist = CalcEurler(startX, startY, endX, endY);
    que.push(make_shared<Node>(startX, startY, 0, dist, nullptr));
    ...
    while (!que.empty()) {
        ...
        for (auto [dx, dy] : dir) {
            ...
            if (xx < 1 || xx > 1000 || yy < 1 || yy > 1000 || visited.count({xx, yy}))
                continue;
            int curDist = CalcEurler(xx, yy, endX, endY);
            que.push(make_shared<Node>(xx, yy, cur->g + 5, curDist, cur));
        }
    }
    return -1;
}
